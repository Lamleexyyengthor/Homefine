...
export const Checkout = () => {
  ...
  const [activeTab, setActiveTab] = useState("/checkout"); // ค่า default
  const handleSave = () => {
  ...
  const tabs = [
    { label: "ຫ້ອງທັງໝົດ", path: "/checkout" },
    { label: "ປະຫວັດການແຈ້ງອອກ", path: "/checkout/historycheckout" },
  ];
  const handleAddss = () => {
  ...
  return (
    <div className=" p-4 bg-gray-50 min-h-screen scrollbar-hide overflow-hidden">
      ...
      <p className="text-[17px] text-[#005E6B]">
        ຈໍານວນຫ້ອງ check-out ທັງໝົດ 9 ຫ້ອງ
      </p>
      {/* oulet */}
      <Outlet />
      {/* Modal */}
